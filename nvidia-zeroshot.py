...
#                            (a) flat list of prompts  -> each prompt is its own class
#                                e.g. ["tanks", "trucks", "cans"]
#                            (b) grouped classes (dict) -> multiple prompts merge into one class
#                                e.g. {
#                                  "vehicle":   ["tanks", "trucks", "cars"],
#                                  "container": ["cans", "soda"]
#                                }
#                            (c) grouped classes (list of dicts) -> same as (b), alt spelling
#                                e.g. [
#                                  {"label": "vehicle",   "prompts": ["tanks", "trucks", "cars"]},
#                                  {"label": "container", "prompts": ["cans", "soda"]}
#                                ]
#   yolo_model_path       str        - path to a .pt YOLO model, used if "YOLO" in models2run
#   models2run            list[str]  - subset of ["DINO","FLORANCE","YOLO","OWL2","YOLOWORLD","LOCATEANYTHING"]
...
#   locateanything_model_path      str   - HF repo id or local path, default "nvidia/LocateAnything-3B"
#   locateanything_generation_mode str   - "fast" | "slow" | "hybrid" (default "hybrid")
#   locateanything_max_new_tokens  int   - default 2048
#   assessment            bool       - print a per-model breakdown (frames/boxes) plus
#                                       source-vs-annotated frame counts after labelling
...
# edit here to change zeroshot config
    ...
    _cfg = {
        ...
        # Flat list example:
        #   "items": ["tanks", "trucks", "vehicles", "cars"],
        # Grouped-class example (multiple prompts merged into one output class):
        "items": {
            "vehicle": ["tanks", "trucks", "vehicles", "cars", "vehicles under bushes", "vehicles under trees"],
        },
        "yolo_model_path": "/home/tables/Documents/TitaThing/TitaSharedHost/TitaModels/model_35_2026-06-03/train/weights/best.pt",
        # models avail: ["OWL2", "DINO", "FLORANCE", "YOLO", "LOCATEANYTHING"]
        "models2run":      ["LOCATEANYTHING"],
        "batch_selection": ["BATCH1"],
        "box_threshold":   0.45,
        "text_threshold":  0.15,
        "merge_nms_iou":   0.4,
        "florence_score_threshold": 0.3,
        "owl_score_threshold":      0.1,
        "dino_type":       "SwinB", # [SwinT | SwinB]
        "annotated":       True, # set to true if you want to render bounding boxes on the images for visual inspection
        # locate anything config
        "locateanything_model_path":      "nvidia/LocateAnything-3B",
        "locateanything_generation_mode": "hybrid",
        "locateanything_max_new_tokens":  2048,
        "assessment":      False,
    }

_raw_items = _cfg.get("items")
if not _raw_items:
    raise ValueError(
        "No 'items' (class/prompt list) provided. Pass via --config "
        "with an 'items' key, e.g. {\"items\": [\"standing soda can\"]} or "
        "{\"items\": [{\"label\": \"vehicle\", \"prompts\": [\"tank\", \"truck\"]}]}"
    )
...
def normalize_items(raw_items):
    ...
    Accepts any of:
      (a) flat list of prompt strings
            ["tanks", "trucks", "cans"]
          -> each prompt becomes its own class

      (b) dict mapping a class label/id straight to its prompt list
            {"1": ["tanks", "trucks"], "2": ["cans", "soda"]}
          -> all prompts under one key merge into that one class
             (dict key is used as the class label; order preserved as given)

      (c) list of {"label": ..., "prompts": [...]} dicts
            [{"label": "vehicle", "prompts": ["tanks", "trucks"]}]
          -> same grouping as (b), alternate spelling

    Returns:
      class_labels   : list[str]          - output class names, index == category_id
      prompt_list    : list[str]          - every raw prompt string, in order (for captions/queries)
      prompt_to_class: dict[str, int]     - lowercased prompt -> category_id
    """
    class_labels    = []
    prompt_list     = []
    prompt_to_class = {}
    ...
    if isinstance(raw_items, dict):
        # {"label_or_id": ["prompt1", "prompt2"], ...}
        for class_id, (label, prompts) in enumerate(raw_items.items()):
            if not prompts:
                raise ValueError(f"Class '{label}' has no prompts.")
            class_labels.append(str(label))
            for p in prompts:
                prompt_list.append(p)
                prompt_to_class[p.lower()] = class_id
        return class_labels, prompt_list, prompt_to_class

    if raw_items and isinstance(raw_items[0], dict):
        # [{"label": ..., "prompts": [...]}, ...]
        for class_id, group in enumerate(raw_items):
            label   = group.get("label") or group.get("name") or f"class_{class_id}"
            prompts = group.get("prompts") or group.get("items") or []
            if not prompts:
                raise ValueError(f"Grouped item {group!r} has no 'prompts' (or 'items') list.")
    ...
    # flat list -> one class per prompt
    class_labels    = list(raw_items)
    prompt_list     = list(raw_items)
    prompt_to_class = {p.lower(): i for i, p in enumerate(raw_items)}
    return class_labels, prompt_list, prompt_to_class


class_labels, prompt_list, prompt_to_class = normalize_items(_raw_items)

# kept for backwards compat with any external code/notes referencing `items` as prompts
items = prompt_list


def resolve_class(phrase: str):
    """
    Matches a model's free-text output phrase against the configured prompts and
    returns the resolved category_id, or None if nothing matches.
    Uses longest-substring match so more specific prompts win over generic ones.
    """
    phrase_l = phrase.lower()
    best_prompt_len = -1
    best_cid = None
    for prompt_l, cid in prompt_to_class.items():
        if prompt_l in phrase_l or phrase_l in prompt_l:
            if len(prompt_l) > best_prompt_len:
                best_prompt_len = len(prompt_l)
                best_cid = cid
    return best_cid


CAMERA_FEED_PATH = os.path.join(BaseDir, "Zeroshot-Images4Labelling")
OUTPUT_PATH      = os.path.join(BaseDir, "Zeroshot-LabelledOutput")
...
GROUNDING_DINO_DIR = _cfg.get("grounding_dino_dir", os.path.join(BaseDir, "LOCALMODELSHERE/GroundingDINO"))
FLORENCE_PATH       = _cfg.get("florence_path", os.path.join(BaseDir, "LOCALMODELSHERE/Florence-2-large"))
...
# NVIDIA LocateAnything-3B config
LOCATEANYTHING_MODEL_PATH      = _cfg.get("locateanything_model_path", "nvidia/LocateAnything-3B")
LOCATEANYTHING_GENERATION_MODE = _cfg.get("locateanything_generation_mode", "hybrid")
LOCATEANYTHING_MAX_NEW_TOKENS  = int(_cfg.get("locateanything_max_new_tokens", 2048))
# Longest side (px) images get downscaled to before hitting the ViT — the model's attention
# memory grows quadratically with patch count, so large source frames OOM easily. Box
# coordinates come back on a fixed 0-1000 normalized grid, so downscaling doesn't affect
# output accuracy in any way that matters for bbox placement. Set to 0 to disable resizing.
LOCATEANYTHING_MAX_IMAGE_SIDE  = int(_cfg.get("locateanything_max_image_side", 1024))
# Sampling controls — LocateAnything is generative (writes <box> tokens as text), so there's
# no per-box confidence score to threshold on like OWL/DINO/YOLO. These control how "loose"
# or repeatable its box guesses are instead:
#   do_sample=False -> greedy decoding, deterministic, usually more conservative/precise
#   do_sample=True  -> samples per temperature/top_p, more boxes but noisier, non-deterministic
LOCATEANYTHING_DO_SAMPLE          = bool(_cfg.get("locateanything_do_sample", True))
LOCATEANYTHING_TEMPERATURE        = float(_cfg.get("locateanything_temperature", 0.7))
LOCATEANYTHING_TOP_P              = float(_cfg.get("locateanything_top_p", 0.9))
LOCATEANYTHING_REPETITION_PENALTY = float(_cfg.get("locateanything_repetition_penalty", 1.1))

# ── assessment toggle ──────────────────────────────────────────────────────────
assessment = bool(_cfg.get("assessment", False))
...
# ── general functions ──────────────────────────────────────────────────────────
def CIP(prompts: list) -> str:
    """Converts prompt list into a GroundingDINO caption string."""
    return " . ".join(prompts) + " ."
...
def count_source_images(batches: list) -> int:
    """Counts total source image files across all selected batches."""
    total = 0
    for batch in batches:
        images = [f for f in os.listdir(batch) if f.lower().endswith((".jpg", ".jpeg", ".png"))]
        total += len(images)
    return total
...
def build_coco_json(model_name: str) -> dict:
    """Returns an empty COCO-style inference JSON skeleton.
       Categories come from the resolved class_labels (grouped or flat)."""
    return {
        "meta": {
        ...
        "categories":  [{"id": i, "name": label} for i, label in enumerate(class_labels)],
        "images":      [],
        "annotations": []
    }
...
def print_assessment_stats(json_paths: list, batches: list, merged_path: str = None):
    """
    Per-model breakdown of how many frames each labeller processed, how many of
    those frames actually got at least one bounding box, and how many boxes it
    produced in total — plus the original source frame count for reference,
    and (if a merge happened) the final merged frame/box counts.
    """
    total_source = count_source_images(batches)

    print("\n── Assessment " + "─" * 45)
    print(f"  Source frames found : {total_source}")

    for json_path in json_paths:
        with open(json_path) as f:
            coco = json.load(f)

        model_name       = coco.get("meta", {}).get("model", os.path.basename(json_path))
        frames_processed = len(coco["images"])
        annotated_ids    = {ann["image_id"] for ann in coco["annotations"]}
        frames_annotated = len(annotated_ids)
        total_boxes      = len(coco["annotations"])

        print(f"  [{model_name}]")
        print(f"      frames processed  : {frames_processed}")
        print(f"      frames annotated  : {frames_annotated}")
        print(f"      bounding boxes    : {total_boxes}")

    if merged_path and os.path.exists(merged_path):
        with open(merged_path) as f:
            merged = json.load(f)
        merged_annotated_ids = {ann["image_id"] for ann in merged["annotations"]}
        print(f"  [merged+nms]")
        print(f"      frames processed  : {len(merged['images'])}")
        print(f"      frames annotated  : {len(merged_annotated_ids)}")
        print(f"      bounding boxes    : {len(merged['annotations'])}")
...
def LabellerFlorence(batches, dataset_path):
    ...
    for p in prompt_list:
        for task_prefix, task_key in FLORENCE_PROMPTS:
            prompt_text = (f"{task_prefix}{p}"
                           if task_prefix == "<OPEN_VOCABULARY_DETECTION>"
                           else f"{task_prefix}a photo of a {p}")
            all_prompts.append((prompt_text, task_key, p))
    ...
    for batch in batches:
        ...
        for fname in tqdm(images, desc=f"Labelling [{batch_name}] [Florence-2]"):
            ...
            for prompt_text, task_key, p in all_prompts:
                inputs = processor(
                ...
                for bbox, score in zip(bboxes, scores):
                    ...
                    bw = (x2-x1) / w
                    bh = (y2-y1) / h
                    detections.append((p, cx, cy, bw, bh, score))

            coco["images"].append({
                "id": image_id, "file_name": fname,
                "source_path": src_path, "width": w, "height": h
            })
            for p, cx, cy, bw, bh, score in detections:
                coco["annotations"].append({
                    "id":           ann_id,
                    "image_id":     image_id,
                    "category_id":  prompt_to_class[p.lower()],
                    "bbox":         [round(cx,6), round(cy,6), round(bw,6), round(bh,6)],
                    "confidence":   score,
        ...
        for i in tqdm(range(0, len(images), YOLO_BATCH_SIZE), desc=f"Labelling [{batch_name}] [YOLO-World]"):
            ...
            for fname, result in zip(batch_fnames, results):
                ...
                for box in result.boxes:
                    cls_idx = int(box.cls)
                    if cls_idx >= len(prompt_list):
                        continue
                    cid = prompt_to_class[prompt_list[cls_idx].lower()]
                    conf = float(box.conf)
...
def LabellerOWL(batches, dataset_path):
    ...
    owl_queries = [f"a photo of a {p}" for p in prompt_list]
        ...
        for i in tqdm(range(0, len(images), OWL_BATCH_SIZE), desc=f"Labelling [{batch_name}] [OWLv2]"):
            ...
            for fname, results, (orig_w, orig_h) in zip(batch_fnames, batch_results, sizes):
                ...
                for box, score, lbl in zip(results["boxes"], results["scores"], results["labels"]):
                    score   = float(score)
                    lbl_idx = int(lbl)
                    if lbl_idx >= len(prompt_list):
                        continue
                    cid = prompt_to_class[prompt_list[lbl_idx].lower()]
...
# LocateAnything returns coords normalised to an integer 0-1000 grid, not 0-1
_LOCATEANYTHING_BOX_RE = re.compile(r"<box><(\d+)><(\d+)><(\d+)><(\d+)></box>")

def _resize_for_locateanything(image: Image.Image) -> Image.Image:
    """Downscales so the longer side is <= LOCATEANYTHING_MAX_IMAGE_SIDE.
       Box outputs are on a fixed 0-1000 grid, so this doesn't skew bbox placement."""
    if LOCATEANYTHING_MAX_IMAGE_SIDE <= 0:
        return image
    w, h = image.size
    longest = max(w, h)
    if longest <= LOCATEANYTHING_MAX_IMAGE_SIDE:
        return image
    scale  = LOCATEANYTHING_MAX_IMAGE_SIDE / longest
    new_wh = (max(1, int(w * scale)), max(1, int(h * scale)))
    return image.resize(new_wh, Image.BILINEAR)

def _locateanything_predict(model, tokenizer, processor, device, dtype, image, question):
    """Runs one image+text query through LocateAnything-3B and returns the raw answer string."""
    messages = [
        {"role": "user", "content": [
    ...
    with torch.no_grad():
        response = model.generate(
            ...
            tokenizer=tokenizer,
            max_new_tokens=LOCATEANYTHING_MAX_NEW_TOKENS,
            use_cache=True,
            generation_mode=LOCATEANYTHING_GENERATION_MODE,
            temperature=LOCATEANYTHING_TEMPERATURE,
            do_sample=LOCATEANYTHING_DO_SAMPLE,
            top_p=LOCATEANYTHING_TOP_P,
            repetition_penalty=LOCATEANYTHING_REPETITION_PENALTY,
            verbose=False,
        )

    return response[0] if isinstance(response, tuple) else response

def _locateanything_parse_boxes(answer: str) -> list:
    """Parses <box><x1><y1><x2><y2></box> tokens (0-1000 grid) into normalised (cx,cy,bw,bh)."""
    boxes = []
    for m in _LOCATEANYTHING_BOX_RE.finditer(answer):
...
def LabellerLocateAnything(batches, dataset_path):
    from transformers import AutoModel, AutoTokenizer, AutoProcessor
    device = "cuda" if torch.cuda.is_available() else "cpu"
    dtype  = torch.bfloat16

    tokenizer = AutoTokenizer.from_pretrained(LOCATEANYTHING_MODEL_PATH, trust_remote_code=True)
    processor = AutoProcessor.from_pretrained(LOCATEANYTHING_MODEL_PATH, trust_remote_code=True)
    model     = AutoModel.from_pretrained(
        LOCATEANYTHING_MODEL_PATH, torch_dtype=dtype, trust_remote_code=True
    ).to(device).eval()

    coco     = build_coco_json(model_name=LOCATEANYTHING_MODEL_PATH)
    image_id = 0
    ann_id   = 0
    ...
    for batch in batches:
        ...
        for fname in tqdm(images, desc=f"Labelling [{batch_name}] [LocateAnything]"):
            src_path  = os.path.join(batch, fname)
            pil_image_orig = Image.open(src_path).convert("RGB")
            w, h           = pil_image_orig.size
            pil_image      = _resize_for_locateanything(pil_image_orig)
            ...
            # one phrase-grounding call per prompt, so each response maps to a known category_id
            for p in prompt_list:
                prompt = f"Locate all the instances that match the following description: {p}."
                try:
                    answer = _locateanything_predict(model, tokenizer, processor, device, dtype, pil_image, prompt)
                except torch.cuda.OutOfMemoryError:
                    torch.cuda.empty_cache()
                    print(f"[WARN]  OOM on {fname} (prompt: '{p}') even after resize — skipping this prompt/image.")
                    continue
                for cx, cy, bw, bh in _locateanything_parse_boxes(answer):
                    coco["annotations"].append({
                        "id":           ann_id,
                        "image_id":     image_id,
                        "category_id":  prompt_to_class[p.lower()],
                        "bbox":         [round(cx,6), round(cy,6), round(bw,6), round(bh,6)],
                        "confidence":   1.0,   # model does not emit a per-box confidence score
                        "source_model": LOCATEANYTHING_MODEL_PATH
                    })
                    ann_id += 1
            image_id += 1
            torch.cuda.empty_cache()

    json_path = os.path.join(dataset_path, "raw", "inference_locateanything.json")
    with open(json_path, "w") as f:
        json.dump(coco, f, indent=2)
    print(f"Saved LocateAnything inference JSON -> {json_path}")
    del model
    torch.cuda.empty_cache()
    return json_path
...
def main():
    ...
    for cid, label in enumerate(class_labels):
        matching_prompts = [p for p, c in prompt_to_class.items() if c == cid]
        print(f"  [{cid}] {label}  <-  {matching_prompts}")
    print("─" * 59)
    ...
    if "LOCATEANYTHING" in models2run:
        json_path_locateanything = LabellerLocateAnything(batches, dataset_path)
        if json_path_locateanything:
            json_paths.append(json_path_locateanything)
    ...
    if assessment:
        print_assessment_stats(json_paths, batches, merged_path if len(json_paths) > 1 else None)
