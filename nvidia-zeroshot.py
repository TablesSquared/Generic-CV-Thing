...
def LabellerLocateAnything(batches, dataset_path):
    ...
    for batch in batches:
        ...
        for fname in tqdm(images, desc=f"Labelling [{batch_name}] [LocateAnything]"):
            ...
            # one phrase-grounding call per item, so each response maps to a known category_id
            for item in items:
                prompt = f"Locate all the instances that match the following description: {item}."
